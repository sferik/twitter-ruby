...
module X
  module REST
    module Search
      ...
      #
      # @see https://dev.X.com/rest/reference/get/search/tweets
      # @see https://dev.X.com/rest/public/search
      # @note Please note that X's search service and, by extension, the Search API is not meant to be an exhaustive source of Tweets. Not all Tweets will be indexed or made available via the search interface.
      # @rate_limited Yes
      ...
      # @option options [String] :geocode Returns tweets by users located within a given radius of the given latitude/longitude. The location is preferentially taking from the Geotagging API, but will fall back to their X profile. The parameter value is specified by "latitude,longitude,radius", where radius units must be specified as either "mi" (miles) or "km" (kilometers). Note that you cannot use the near operator via the API to geocode arbitrary locations; however you can use this geocode parameter to search near geocodes directly.
      ...
      # @return [X::SearchResults] Return tweets that match a specified query with search metadata
      def search(query, options = {})
        options = options.dup
        options[:count] ||= MAX_TWEETS_PER_REQUEST
        request = X::REST::Request.new(self, :get, "/1.1/search/tweets.json", options.merge(q: query))
        X::SearchResults.new(request)
      end
    end
  end
end
