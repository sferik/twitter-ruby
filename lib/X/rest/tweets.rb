...
module X
  module REST
    module Tweets
      ...
      #
      # @see https://dev.X.com/rest/reference/get/statuses/retweets/:id
      # @rate_limited Yes
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Array<X::Tweet>]
      # @param tweet [Integer, String, URI, X::Tweet] A Tweet ID, URI, or object.
      # @param options [Hash] A customizable set of options.
      # @option options [Integer] :count Specifies the number of records to retrieve. Must be less than or equal to 100.
      # @option options [Boolean, String, Integer] :trim_user Each tweet returned in a timeline will include a user object with only the author's numerical ID when set to true, 't' or 1.
      def retweets(tweet, options = {})
        perform_get_with_objects("/1.1/statuses/retweets/#{extract_id(tweet)}.json", options, X::Tweet)
      end
      ...
      # Returns a Tweet
      #
      # @see https://dev.X.com/rest/reference/get/statuses/show/:id
      # @rate_limited Yes
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @raise [X::Error::Forbidden] Error raised when supplied status is over 280 characters.
      # @return [X::Tweet] The requested Tweet.
      # @param tweet [Integer, String, URI, X::Tweet] A Tweet ID, URI, or object.
      # @param options [Hash] A customizable set of options.
      # @option options [Boolean, String, Integer] :trim_user Each tweet returned in a timeline will include a user object with only the author's numerical ID when set to true, 't' or 1.
      def status(tweet, options = {})
        perform_get_with_object("/1.1/statuses/show/#{extract_id(tweet)}.json", options, X::Tweet)
      end

      # Returns Tweets
      #
      # @see https://dev.X.com/rest/reference/get/statuses/lookup
      # @rate_limited Yes
      # @authentication Required
      # @return [Array<X::Tweet>] The requested Tweets.
      ...
      def statuses(*args)
        ...
        flat_pmap(arguments.each_slice(MAX_TWEETS_PER_REQUEST)) do |tweets|
          perform_post_with_objects("/1.1/statuses/lookup.json", arguments.options.merge(id: tweets.collect { |u| extract_id(u) }.join(",")), X::Tweet)
        end
      end

      # Destroys the specified Tweets
      #
      # @see https://dev.X.com/rest/reference/post/statuses/destroy/:id
      # @note The authenticating user must be the author of the specified Tweets.
      # @rate_limited No
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Array<X::Tweet>] The deleted Tweets.
      ...
      def destroy_status(*args)
        ...
        pmap(arguments) do |tweet|
          perform_post_with_object("/1.1/statuses/destroy/#{extract_id(tweet)}.json", arguments.options, X::Tweet)
        end
      end
      ...
      #
      # @see https://dev.X.com/rest/reference/post/statuses/update
      ...
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [X::Tweet] The created Tweet. When the tweet is deemed a duplicate by X, returns the last Tweet from the user's timeline.
      ...
      # @option options [X::Tweet] :in_reply_to_status An existing status that the update is in reply to. If the status being replied to was not originally posted by the authenticated user, the text of the status must begin with an @-mention, or X will reject the update.
      ...
      # @option options [X::Place] :place A place in the world. These can be retrieved from {X::REST::PlacesAndGeo#reverse_geocode}.
      # @option options [String] :place_id A place in the world. These IDs can be retrieved from {X::REST::PlacesAndGeo#reverse_geocode}.
      # @option options [String] :display_coordinates Whether or not to put a pin on the exact coordinates a tweet has been sent from.
      # @option options [Boolean, String, Integer] :trim_user Each tweet returned in a timeline will include a user object with only the author's numerical ID when set to true, 't' or 1.
      def update(status, options = {})
        update!(status, options)
      rescue X::Error::DuplicateStatus
      ...
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @raise [X::Error::DuplicateStatus] Error raised when a duplicate status is posted.
      # @return [X::Tweet] The created Tweet.
      # @param status [String] The text of your status update, up to 280 characters.
      # @param options [Hash] A customizable set of options.
      # @option options [Boolean, String, Integer] :possibly_sensitive Set to true for content which may not be suitable for every audience.
      # @option options [X::Tweet] :in_reply_to_status An existing status that the update is in reply to. If the status being replied to was not originally posted by the authenticated user, the text of the status must begin with an @-mention, or X will reject the update.
      ...
      def update!(status, options = {})
        ...
        perform_post_with_object("/1.1/statuses/update.json", hash.merge(status: status), X::Tweet)
      end

      # Retweets the specified Tweets as the authenticating user
      #
      # @see https://dev.X.com/rest/reference/post/statuses/retweet/:id
      # @rate_limited Yes
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Array<X::Tweet>] The original tweets with retweet details embedded.
      ...
      def retweet(*args)
        ...
        rescue X::Error::AlreadyRetweeted, X::Error::NotFound
          next
        end.compact
      end
      ...
      # @rate_limited Yes
      # @authentication Requires user context
      # @raise [X::Error::AlreadyRetweeted] Error raised when tweet has already been retweeted.
      ...
      #
      # @see https://dev.X.com/rest/reference/post/statuses/update_with_media
      ...
      # @param options [Hash] A customizable set of options.
      # @option options [Boolean, String, Integer] :possibly_sensitive Set to true for content which may not be suitable for every audience.
      # @option options [X::Tweet] :in_reply_to_status An existing status that the update is in reply to.
      ...
      #
      # @see https://dev.X.com/rest/reference/get/statuses/oembed
      # @rate_limited Yes
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [X::OEmbed] OEmbed for the requested Tweet.
      # @param tweet [Integer, String, URI, X::Tweet] A Tweet ID, URI, or object.
      # @param options [Hash] A customizable set of options.
      ...
      # @option options [Boolean, String, Integer] :hide_media Specifies whether the embedded Tweet should automatically expand images which were uploaded via {https://dev.X.com/rest/reference/post/statuses/update_with_media POST statuses/update_with_media}. When set to either true, t or 1 images will not be expanded. Defaults to false.
      ...
      # @option options [String] :related A value for the TWT related parameter, as described in {https://dev.X.com/web/intents Web Intents}. This value will be forwarded to all Web Intents calls.
      # @option options [String] :lang Language code for the rendered embed. This will affect the text and localization of the rendered HTML.
      # @option options [String] :widget_type Set to video to return a X Video embed for the given Tweet.
      # @option options [Boolean, String] :hide_tweet Applies to video type only. Set to 1 or true to link directly to the Tweet URL instead of displaying a Tweet overlay when a viewer clicks on the X bird logo.
      def oembed(tweet, options = {})
        options = options.dup
        options[:id] = extract_id(tweet)
        perform_get_with_object("/1.1/statuses/oembed.json", options, X::OEmbed)
      end
      ...
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Array<X::OEmbed>] OEmbeds for the requested Tweets.
      ...
      #
      # @see https://dev.X.com/rest/reference/get/statuses/retweeters/ids
      # @rate_limited Yes
      # @authentication Required
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [X::Cursor]
      ...
      #
      # @see https://dev.X.com/rest/reference/post/statuses/unretweet/:id
      # @rate_limited Yes
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Array<X::Tweet>] The original tweets with retweet details embedded.
