...
module X
  module REST
    module PlacesAndGeo
      ...
      #
      # @see https://dev.X.com/rest/reference/get/geo/id/:place_id
      # @rate_limited Yes
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @param place_id [String] A place in the world. These IDs can be retrieved from {X::REST::PlacesAndGeo#reverse_geocode}.
      # @param options [Hash] A customizable set of options.
      # @return [X::Place] The requested place.
      def place(place_id, options = {})
        perform_get_with_object("/1.1/geo/id/#{place_id}.json", options, X::Place)
      end

      # Searches for up to 20 places that can be used as a place_id
      #
      # @see https://dev.X.com/rest/reference/get/geo/reverse_geocode
      ...
      def reverse_geocode(options = {})
        perform_get_with_object("/1.1/geo/reverse_geocode.json", options, X::GeoResults)
      end

      # Search for places that can be attached to a {X::REST::Tweets#update}
      #
      # @see https://dev.X.com/rest/reference/get/geo/search
      # @rate_limited Yes
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @param options [Hash] A customizable set of options.
      ...
      def geo_search(options = {})
        perform_get_with_object("/1.1/geo/search.json", options, X::GeoResults)
      end
      alias places_nearby geo_search

      # Locates places near the given coordinates which are similar in name
      #
      # @see https://dev.X.com/rest/reference/get/geo/similar_places
      # @note Conceptually, you would use this method to get a list of known places to choose from first. Then, if the desired place doesn't exist, make a request to {X::REST::PlacesAndGeo#place} to create a new one. The token contained in the response is the token necessary to create a new place.
      # @rate_limited Yes
      # @authentication Requires user context
      # @raise [X::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @param options [Hash] A customizable set of options.
      ...
      def similar_places(options = {})
        perform_get_with_object("/1.1/geo/similar_places.json", options, X::GeoResults)
      end
