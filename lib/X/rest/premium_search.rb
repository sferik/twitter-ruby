...
module X
  module REST
    module PremiumSearch
      ...
      #
      # @see https://developer.X.com/en/docs/tweets/search/overview/premium
      # @see https://developer.X.com/en/docs/tweets/search/api-reference/premium-search.html#DataEndpoint
      # @rate_limited Yes
      ...
      # @return [X::PremiumSearchResults] Return tweets that match a specified query with search metadata
      def premium_search(query, options = {}, request_config = {})
        ...
        request = X::REST::Request.new(self, request_config[:request_method], path, options.merge(query: query))
        X::PremiumSearchResults.new(request, request_config)
      end
    end
  end
end
