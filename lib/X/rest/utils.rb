...
module X
  module REST
    ...
    private
      ...
      # Take a URI string or X::Identity object and return its ID
      #
      # @param object [Integer, String, URI, X::Identity] An ID, URI, or object.
      # @return [Integer]
      ...
      def perform_request(request_method, path, options = {}, params = nil)
        X::REST::Request.new(self, request_method, path, options, params).perform
      end
      ...
      def perform_get_with_cursor(path, options, collection_name, klass = nil)
        ...
        request = X::REST::Request.new(self, :get, path, options)
        X::Cursor.new(collection_name.to_sym, klass, request, limit)
      end
      ...
      def parallel_users_from_response(request_method, path, args)
        ...
        pmap(arguments) do |user|
          perform_request_with_object(request_method, path, merge_user(arguments.options, user), X::User)
        end
      end
      ...
      def users_from_response(request_method, path, args)
        ...
        perform_request_with_objects(request_method, path, arguments.options, X::User)
      end
      ...
      #
      # @param hash [Hash]
      # @param users [Enumerable<Integer, String, X::User>] A collection of X user IDs, screen_names, or objects.
      # @return [Hash]
      ...
      #
      # @param hash [Hash]
      # @param users [Enumerable<Integer, String, URI, X::User>] A collection of X user IDs, screen_names, URIs, or objects.
      # @return [Hash]
      def merge_users!(hash, users)
      ...
      def collect_users(users) # rubocop:disable Metrics/MethodLength
        ...
        users.each do |user|
          ...
          when X::User         then user_ids << user.id
