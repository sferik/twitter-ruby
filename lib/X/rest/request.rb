...
require "X/utils"
require "X/rest/form_encoder"

module X
  module REST
    class Request # rubocop:disable Metrics/ClassLength
      include X::Utils
      BASE_URL = "https://api.X.com".freeze
      attr_accessor :client, :headers, :options, :path, :rate_limit,
                    :request_method, :uri
      alias verb request_method

      # @param client [X::Client]
      # @param request_method [String, Symbol]
      # @param path [String]
      # @param options [Hash]
      # @return [X::REST::Request]
    ...
    private
      ...
      def request_options
        options = if @options_key == :form
                    {form: HTTP::FormData.create(@options, encoder: X::REST::FormEncoder.method(:encode))}
                  else
                    {@options_key => @options}
                  end
      ...
      def set_multipart_options!(request_method, options)
        ...
        @headers = X::Headers.new(@client, @request_method, @uri, options).request_headers
      end
      ...
      def fail_or_return_response_body(code, body, headers)
        ...
        @rate_limit = X::RateLimit.new(headers)
        body
      end

      def error(code, body, headers)
        klass = X::Error::ERRORS[code]
        if klass == X::Error::Forbidden
          forbidden_error(body, headers)
        elsif !klass.nil?
          klass.from_response(body, headers)
        elsif body.is_a?(Hash) && (err = body.dig(:processing_info, :error))
          X::Error::MediaError.from_processing_response(err, headers)
        end
      end

      def forbidden_error(body, headers)
        error = X::Error::Forbidden.from_response(body, headers)
        klass = X::Error::FORBIDDEN_MESSAGES[error.message]
        if klass
          klass.from_response(body, headers)
        else
          error
      ...
      # Returns boolean indicating if all the keys required by HTTP::Client are present in X::Client#timeouts
      #
      # @return [Boolean]
