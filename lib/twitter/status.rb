...
module Twitter
  class Status < Twitter::Identity
    ...
    def geo
      @geo ||= Twitter::GeoFactory.fetch_or_store(@attrs[:geo]) unless @attrs[:geo].nil?
    end
    ...
    def hashtags
      @hashtags ||= unless @attrs[:entities].nil?
        Array(@attrs[:entities][:hashtags]).map do |hashtag|
          Twitter::Entity::Hashtag.fetch_or_store(hashtag)
        end
      else
    ...
    def media
      @media ||= unless @attrs[:entities].nil?
        Array(@attrs[:entities][:media]).map do |media|
          Twitter::MediaFactory.fetch_or_store(media)
        end
      else
    ...
    def metadata
      @metadata ||= Twitter::Metadata.fetch_or_store(@attrs[:metadata]) unless @attrs[:metadata].nil?
    end
    ...
    def place
      @place ||= Twitter::Place.fetch_or_store(@attrs[:place]) unless @attrs[:place].nil?
    end
    ...
    def retweeted_status
      @retweeted_status ||= self.class.fetch_or_store(@attrs[:retweeted_status]) unless @attrs[:retweeted_status].nil?
    end
    ...
    def urls
      @urls ||= unless @attrs[:entities].nil?
        Array(@attrs[:entities][:urls]).map do |url|
          Twitter::Entity::Url.fetch_or_store(url)
        end
      else
    ...
    # @return [Twitter::User]
    def user
      @user ||= Twitter::User.fetch_or_store(@attrs.dup[:user].merge(:status => @attrs.except(:user))) unless @attrs[:user].nil?
    end
    ...
    def user_mentions
      @user_mentions ||= unless @attrs[:entities].nil?
        Array(@attrs[:entities][:user_mentions]).map do |user_mention|
          Twitter::Entity::UserMention.fetch_or_store(user_mention)
        end
      else
