  ...
  module REST
    module AccountActivity
      ...
      def create_webhook(env_name, url)
        perform_request(:json_post, "/2/account_activity/all/#{env_name}/webhooks.json?url=#{url}")
      end
      ...
      def list_webhooks(env_name)
        perform_request(:get, "/2/account_activity/all/#{env_name}/webhooks.json")
      end

      # Removes the webhook from the provided application's all activities configuration. The webhook ID can be accessed by making a call to GET /2/account_activity/all/webhooks.
      ...
      def delete_webhook(env_name, webhook_id)
        perform_request(:delete, "/2/account_activity/all/#{env_name}/webhooks/#{webhook_id}.json")
      end
      ...
      def trigger_crc_check(env_name, webhook_id)
        perform_request(:json_put, "/2/account_activity/all/#{env_name}/webhooks/#{webhook_id}.json")
      end
      ...
      def create_subscription(env_name)
        perform_request(:json_post, "/2/account_activity/all/#{env_name}/subscriptions.json")
      end
      ...
      def check_subscription(env_name)
        perform_request(:get, "/2/account_activity/all/#{env_name}/subscriptions.json")
      end
      ...
      def deactivate_subscription(env_name)
        perform_request(:delete, "/2/account_activity/all/#{env_name}/subscriptions.json")
      end
    end
  end
end
