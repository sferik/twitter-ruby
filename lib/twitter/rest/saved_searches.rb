  ...
  module REST
    module SavedSearches
      ...
      def saved_searches(*args)
        ...
        if arguments.empty?
          perform_get_with_objects("/2/saved_searches/list.json", arguments.options, Twitter::SavedSearch)
        else
      ...
      def saved_search(id, options = {})
        perform_get_with_object("/2/saved_searches/show/#{id}.json", options, Twitter::SavedSearch)
      end
      ...
      def create_saved_search(query, options = {})
        perform_post_with_object("/2/saved_searches/create.json", options.merge(query: query), Twitter::SavedSearch)
      end
      ...
      def destroy_saved_search(*args)
        arguments = Twitter::Arguments.new(args)
        pmap(arguments) do |id|
          perform_post_with_object("/2/saved_searches/destroy/#{id}.json", arguments.options, Twitter::SavedSearch)
        end
      end
    end
  end
