  ...
  module REST
    module Trends
      ...
      def trends(id = 1, options = {})
        options = options.dup
        options[:id] = id
        response = perform_get("/2/trends/place.json", options).first
      ...
      def trends_available(options = {})
        perform_get_with_objects("/2/trends/available.json", options, Twitter::Place)
      end
      ...
      def trends_closest(options = {})
        perform_get_with_objects("/2/trends/closest.json", options, Twitter::Place)
      end
    end
  end
end
