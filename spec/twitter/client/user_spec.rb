...
describe Twitter::Client do
  ...
  describe ".friends" do
    ...
    context "with a screen name passed" do
      ...
      before do
        stub_get("/1/statuses/friends.json").
          with(:query => {:screen_name => "sferik", :cursor => "-1"}).
          to_return(:body => fixture("friends.json"), :headers => {:content_type => "application/json; charset=utf-8"})
      end

      it "should get the correct resource" do
        @client.friends("sferik")
        a_get("/1/statuses/friends.json").
          with(:query => {:screen_name => "sferik", :cursor => "-1"}).
          should have_been_made
      end
  ...
  describe ".followers" do
    ...
    context "with a screen name passed" do
      ...
      before do
        stub_get("/1/statuses/followers.json").
          with(:query => {:screen_name => "sferik", :cursor => "-1"}).
          to_return(:body => fixture("followers.json"), :headers => {:content_type => "application/json; charset=utf-8"})
      end

      it "should get the correct resource" do
        @client.followers("sferik")
        a_get("/1/statuses/followers.json").
          with(:query => {:screen_name => "sferik", :cursor => "-1"}).
          should have_been_made
      end
