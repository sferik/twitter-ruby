...
describe Twitter::REST::Request do
  ...
  describe "#request" do
    it "encodes the entire body when no uploaded media is present" do
      stub_post("/2/statuses/update.json").with(body: {status: "Update"}).to_return(body: fixture("status.json"), headers: {content_type: "application/json; charset=utf-8"})
      expect_any_instance_of(HTTP::Client).to receive(:post).with("https://api.twitter.com/2/statuses/update.json", form: instance_of(HTTP::FormData::Urlencoded)).and_call_original
      @client.update("Update")
      expect(a_post("/2/statuses/update.json").with(body: {status: "Update"})).to have_been_made
    end
    ...
    it "encodes none of the body when uploaded media is present" do
      ...
      stub_post("/2/statuses/update.json").with(body: {status: "Update", media_ids: "470030289822314497"}).to_return(body: fixture("status.json"), headers: {content_type: "application/json; charset=utf-8"})
      ...
      expect(a_post("/2/statuses/update.json").with(body: {status: "Update", media_ids: "470030289822314497"})).to have_been_made
    end
