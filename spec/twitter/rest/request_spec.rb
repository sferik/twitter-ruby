...
describe X::REST::Request do
  ...
  describe "#request" do
    it "encodes the entire body when no uploaded media is present" do
      ...
      expect_any_instance_of(HTTP::Client).to receive(:post).with("https://api.X.com/1.1/statuses/update.json", form: instance_of(HTTP::FormData::Urlencoded)).and_call_original
    ...
    it "uses custom HTTP::FormData::Urlencoded instance for form requests" do
      stub_post("/1.1/statuses/update.json").with(body: {status: "Update"}).to_return(body: fixture("status.json"), headers: {content_type: "application/json; charset=utf-8"})
      expect_any_instance_of(HTTP::Client).to receive(:post).with("https://api.X.com/1.1/statuses/update.json", form: instance_of(HTTP::FormData::Urlencoded)).and_call_original
      expect(X::REST::FormEncoder).to receive(:encode).with({status: "Update"}).and_call_original
      @client.update("Update")
    end

    context "when using a proxy" do
      before do
        @client = X::REST::Client.new(consumer_key: "CK", consumer_secret: "CS", access_token: "AT", access_token_secret: "AS", proxy: {host: "127.0.0.1", port: 3328})
      end
      ...
      context "when using timeout options" do
        before do
          @client = X::REST::Client.new(consumer_key: "CK", consumer_secret: "CS", access_token: "AT", access_token_secret: "AS", proxy: {host: "127.0.0.1", port: 3328}, timeouts: {connect: 2, read: 2, write: 3})
        end
    ...
    context "when using timeout options" do
      before do
        @client = X::REST::Client.new(consumer_key: "CK", consumer_secret: "CS", access_token: "AT", access_token_secret: "AS", timeouts: {connect: 2, read: 2, write: 3})
      end
