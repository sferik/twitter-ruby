...
describe X::REST::Tweets do
  ...
  describe "#statuses" do
    ...
    context "with URI objects passed" do
      it "requests the correct resource" do
        @client.statuses(URI.parse("https://X.com/sferik/status/540897316908331009"), URI.parse("https://X.com/sferik/status/91151181040201728"))
        expect(a_post("/1.1/statuses/lookup.json").with(body: {id: "540897316908331009,91151181040201728"})).to have_been_made
      end
    end
    ...
    context "with Tweets passed" do
      it "requests the correct resource" do
        @client.statuses(X::Tweet.new(id: 540_897_316_908_331_009), X::Tweet.new(id: 91_151_181_040_201_728))
        expect(a_post("/1.1/statuses/lookup.json").with(body: {id: "540897316908331009,91151181040201728"})).to have_been_made
      end
    end
  end
  ...
  describe "#update" do
    ...
    context "with a place" do
      before do
        @place = X::Place.new(woeid: "df51dec6f4ee2b2c")
  ...
  describe "#update!" do
    ...
    context "already posted" do
      ...
      it "raises an DuplicateStatus error" do
        expect { @client.update!("Powerful cartoon by @BillBramhall: http://t.co/IOEbc5QoES") }.to raise_error(X::Error::DuplicateStatus)
      end
    end
  ...
  describe "#retweet!" do
    ...
    context "forbidden" do
      ...
      it "raises a Forbidden error" do
        expect { @client.retweet!(540_897_316_908_331_009) }.to raise_error(X::Error::Forbidden)
      end
    end
    ...
    context "already retweeted" do
      ...
      it "raises an AlreadyRetweeted error" do
        expect { @client.retweet!(540_897_316_908_331_009) }.to raise_error(X::Error::AlreadyRetweeted)
      end
    end
    ...
    context "not found" do
      ...
      it "raises a NotFound error" do
        expect { @client.retweet!(540_897_316_908_331_009) }.to raise_error(X::Error::NotFound)
      end
    end
  ...
  describe "#update_with_media" do
      ...
      context "when the processing is not finished right after the upload" do
        context "when it succeeds" do
          it "asks for status until the processing is done" do
            ...
            expect_any_instance_of(X::REST::DirectMessages).to receive(:sleep).with(5).and_return(5)
            expect_any_instance_of(X::REST::DirectMessages).to receive(:sleep).with(10).and_return(10)
            @client.update_with_media("You always have options", fixture("1080p.mp4"))
            expect(a_request(:post, "https://upload.X.com/1.1/media/upload.json")).to have_been_made.times(3)
        ...
        context "when it fails" do
          it "raises an error" do
            ...
            expect_any_instance_of(X::REST::DirectMessages).to receive(:sleep).with(5).and_return(5)
            expect { @client.update_with_media("You always have options", fixture("1080p.mp4")) }.to raise_error(X::Error::InvalidMedia)
            expect(a_request(:post, "https://upload.X.com/1.1/media/upload.json")).to have_been_made.times(3)
        ...
        context "when X::Client#timeouts[:upload] is set" do
          ...
          it "raises an error when the finalize step is too slow" do
            ...
            finalize_request = {body: fixture("chunk_upload_finalize_pending.json"), headers: {content_type: "application/json; charset=utf-8"}}
            stub_request(:post, "https://upload.X.com/1.1/media/upload.json").to_return(init_request, append_request, finalize_request)
            expect { @client.update_with_media("You always have options", fixture("1080p.mp4")) }.to raise_error(X::Error::TimeoutError)
            expect(a_request(:post, "https://upload.X.com/1.1/media/upload.json")).to have_been_made.times(3)
          end
        end
      end
    ...
    context "with multiple images" do
      it "requests the correct resource" do
        ...
        expect(a_request(:post, "https://upload.X.com/1.1/media/upload.json")).to have_been_made.times(2)
        expect(a_post("/1.1/statuses/update.json")).to have_been_made
      end
    end
  end
  ...
  describe "#oembed" do
    before do
      ...
      stub_get("/1.1/statuses/oembed.json").with(query: {url: "https://X.com/sferik/status/540897316908331009"}).to_return(body: fixture("oembed.json"), headers: {content_type: "application/json; charset=utf-8"})
    end
    ...
    it "requests the correct resource when a URL is given" do
      @client.oembed("https://X.com/sferik/status/540897316908331009")
      expect(a_get("/1.1/statuses/oembed.json").with(query: {id: "540897316908331009"})).to have_been_made
    end

    it "returns an array of OEmbed instances" do
      oembed = @client.oembed(540_897_316_908_331_009)
      expect(oembed).to be_a X::OEmbed
    end
  ...
  describe "#oembeds" do
    ...
    it "requests the correct resource when a URL is given" do
      @client.oembeds("https://X.com/sferik/status/540897316908331009")
      expect(a_get("/1.1/statuses/oembed.json").with(query: {id: "540897316908331009"})).to have_been_made
    end
    ...
    it "returns an array of OEmbed instances" do
      ...
      expect(oembeds.first).to be_a X::OEmbed
    end
  ...
  describe "#retweeters_ids" do
    ...
    it "returns a collection of up to 100 user IDs belonging to users who have retweeted the tweet specified by the id parameter" do
      ...
      expect(retweeters_ids).to be_a X::Cursor
