...
describe Twitter::Error::ServerError do
  ...
  describe "#ratelimit_reset" do
    it "returns a Time when X-RateLimit-Reset header is set" do
      error = Twitter::Error::ServerError.from_env({:response_headers => {'X-RateLimit-Reset' => "1339019097"}})
    ...
    it "returns nil when X-RateLimit-Reset header is not set" do
      error = Twitter::Error::ServerError.from_env({:response_headers => {}})
  ...
  describe "#ratelimit_class" do
    it "returns a String when X-RateLimit-Class header is set" do
      error = Twitter::Error::ServerError.from_env({:response_headers => {'X-RateLimit-Class' => "api"}})
  ...
  describe "#ratelimit_limit" do
    it "returns an Integer when X-RateLimit-Limit header is set" do
      error = Twitter::Error::ServerError.from_env({:response_headers => {'X-RateLimit-Limit' => "150"}})
  ...
  describe "#ratelimit_remaining" do
    it "returns an Integer when X-RateLimit-Remaining header is set" do
      error = Twitter::Error::ServerError.from_env({:response_headers => {'X-RateLimit-Remaining' => "149"}})
