...
describe Twitter::Photo do
  ...
  describe "#sizes" do
    it "returns a hash of Sizes when sizes is set" do
      sizes = Twitter::Photo.new(:id => 110102452988157952, :sizes => {:small => {:h => 226, :w => 340, :resize => 'fit'}, :large => {:h => 466, :w => 700, :resize => 'fit'}, :medium => {:h => 399, :w => 600, :resize => 'fit'}, :thumb => {:h => 150, :w => 150, :resize => 'crop'}}).sizes
      sizes.should be_a Hash
      sizes[:small].should be_a Twitter::Size
    end
    it "is empty when sizes is not set" do
      sizes = Twitter::Photo.new(:id => 110102452988157952).sizes
