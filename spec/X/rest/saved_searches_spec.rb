...
describe X::REST::SavedSearches do
  before do
  ...
  describe "#saved_searches" do
    context "with ids passed" do
      ...
      it "returns an array of saved searches" do
        ...
        expect(saved_searches.first).to be_a X::SavedSearch
        expect(saved_searches.first.name).to eq("X")
        expect(saved_searches.last).to be_a X::SavedSearch
        expect(saved_searches.last.name).to eq("X")
      end
    end
  ...
  describe "#saved_search" do
    ...
    it "returns a saved search" do
      ...
      expect(saved_search).to be_a X::SavedSearch
      expect(saved_search.name).to eq("X")
    end
  end

  describe "#create_saved_search" do
    before do
      stub_post("/1.1/saved_searches/create.json").with(body: {query: "X"}).to_return(body: fixture("saved_search.json"), headers: {content_type: "application/json; charset=utf-8"})
    end

    it "requests the correct resource" do
      @client.create_saved_search("X")
      expect(a_post("/1.1/saved_searches/create.json").with(body: {query: "X"})).to have_been_made
    end

    it "returns the created saved search" do
      saved_search = @client.create_saved_search("X")
      expect(saved_search).to be_a X::SavedSearch
      expect(saved_search.name).to eq("X")
    end
  end
