...
describe Twitter::SearchResults do
  ...
  describe "#attrs=" do
    it "uses collect to return mapped results" do
      results = Twitter::SearchResults.allocate
      results.instance_variable_set(:@collection, [])
      results.instance_variable_set(:@attrs, {})

      return_value = results.send(:attrs=, {statuses: [{id: 1}]})

      assert_kind_of(Array, return_value.first)
    end
  end

  describe "#query_string_to_hash" do
    it "handles nil query by converting with to_s" do
      results = Twitter::SearchResults.allocate

      hash = results.send(:query_string_to_hash, "http://example.com")
