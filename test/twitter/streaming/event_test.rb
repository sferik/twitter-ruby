...
describe Twitter::Streaming::Event do
  describe "#initialize" do
    ...
    it "sets the name to be a symbolised version of string string" do
      assert_equal(:follow, event.name)
    end

    it "sets the source to be the User" do
      assert_kind_of(Twitter::User, event.source)
      assert_equal(10_083_602, event.source.id)
    end

    it "sets the target to be the User" do
      assert_kind_of(Twitter::User, event.target)
      assert_equal(1_292_911_088, event.target.id)
    end

    it "uses hash defaults when the event key is missing" do
      data = Hash.new("follow")
      data[:source] = {id: 10_083_602}
      data[:target] = {id: 1_292_911_088}
      data[:target_object] = nil

      event = Twitter::Streaming::Event.new(data)
    ...
    it "uses Twitter::User explicitly even if Event::User exists" do
      with_stubbed_const("Twitter::Streaming::Event::User", Class.new do
        def self.new(*)
          raise "wrong constant"
        end
      end) do
        data = {event: "follow", source: {id: 10_083_602}, target: {id: 1_292_911_088}, target_object: nil}
        assert_nothing_raised { Twitter::Streaming::Event.new(data) }
      end
    end

    it "uses Twitter::List explicitly even if Event::List exists" do
      with_stubbed_const("Twitter::Streaming::Event::List", Class.new do
        def self.new(*)
          raise "wrong constant"
        end
      end) do
        data = {event: "list_member_added", source: {id: 10_083_602}, target: {id: 1_292_911_088}, target_object: {id: 1}}
        assert_nothing_raised { Twitter::Streaming::Event.new(data) }
      end
    end

    it "uses Twitter::Tweet explicitly even if Event::Tweet exists" do
      with_stubbed_const("Twitter::Streaming::Event::Tweet", Class.new do
        def self.new(*)
          raise "wrong constant"
        end
      end) do
        data = {event: "favorite", source: {id: 10_083_602}, target: {id: 1_292_911_088}, target_object: {id: 1}}
        assert_nothing_raised { Twitter::Streaming::Event.new(data) }
      end
    end

    describe "when target object is a list" do
      before do
      ...
      it "has the list object as the target object" do
        assert_kind_of(Twitter::List, event.target_object)
        assert_equal("@adambird/lists/dev", event.target_object.full_name)
      end
    end

    describe "when target object is a tweet" do
      before do
      ...
      it "has the tweet as the target object" do
        assert_kind_of(Twitter::Tweet, event.target_object)
        assert_equal(394_454_214_132_256_768, event.target_object.id)
      end
    end
  end
end
