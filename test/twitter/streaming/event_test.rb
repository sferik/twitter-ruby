...
describe Twitter::Streaming::Event do
  describe "#initialize" do
    ...
    it "uses hash defaults when the event key is missing" do
      ...
      expect(event.name).to eq(:follow)
    end
    ...
    it "uses hash defaults when the source key is missing" do
      ...
      event = described_class.new(data)
      expect(event.source.id).to eq(10_083_602)
    end
    ...
    it "uses hash defaults when the target key is missing" do
      ...
      event = described_class.new(data)
      expect(event.target.id).to eq(1_292_911_088)
    end

    it "uses Twitter::User explicitly even if Event::User exists" do
      stub_const("Twitter::Streaming::Event::User", Class.new do
        def self.new(*)
    ...
    it "uses Twitter::List explicitly even if Event::List exists" do
      stub_const("Twitter::Streaming::Event::List", Class.new do
        def self.new(*)
          raise "wrong constant"
        end
      end)
    ...
    it "uses Twitter::Tweet explicitly even if Event::Tweet exists" do
      stub_const("Twitter::Streaming::Event::Tweet", Class.new do
        def self.new(*)
          raise "wrong constant"
        end
      end)
