...
describe Twitter::REST::OAuth do
  ...
  describe "#reverse_token" do
    ...
    it "uses oauth_auth_header.to_s as the authorization header" do
      ...
      oauth_header = Class.new do
        def to_s = "OAuth oauth_consumer_key=\"CK\""
      end.new
      header_builder = Class.new do
        define_method(:oauth_auth_header) { oauth_header }
      end.new
      http_client = Object.new
      response = Class.new do
        def to_s = "oauth_token=abc"
      end.new
      headers_new_called = false
      http_headers_called = false
      post_called = false
