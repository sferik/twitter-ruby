...
describe Twitter::REST::Favorites do
  ...
  describe "#favorites" do
    ...
    describe "with multiple positional arguments" do
      before do
        stub_get("/1.1/favorites/list.json").with(query: {screen_name: "last_user"}).to_return(body: fixture("user_timeline.json"), headers: json_headers)
      end

      it "uses the last positional argument as the user" do
        @client.favorites("first_user", "last_user")

        assert_requested(a_get("/1.1/favorites/list.json").with(query: {screen_name: "last_user"}))
      end
    end
